# Shared Documentation

These documents are maintained by the `apexplane.roles` collection and copied into consuming infrastructure repositories during `task install` and `task upgrade`.

Edit the source documents in `apexplane-ansible-roles`, not in consuming repositories.

## Reference

- [Ansible conventions](reference/ansible-conventions.md)
- [Migration playbooks](reference/migrations.md)
- [Port conventions](reference/port-conventions.md)
- [Role contracts](reference/role-contracts.md)

## Runbooks

- [Bootstrap](runbooks/bootstrap.md)
- [CrowdSec](runbooks/crowdsec.md)
- [Maintenance](runbooks/maintenance.md)
- [Toolbox](runbooks/toolbox.md)

## Services

- [Elastic](services/elastic.md)
- [GitLab Runner](services/gitlab-runner.md)
- [Loki](services/loki.md)
- [MinIO](services/minio.md)
- [PostgreSQL](services/postgresql.md)
- [PostgreSQL replica](services/postgresql-replica.md)
- [ZITADEL](services/zitadel.md)
