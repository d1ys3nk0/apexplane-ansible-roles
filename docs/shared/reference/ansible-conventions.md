...
- Shared roles are consumed as the `apexplane.roles` collection from generated `requirements.yml`.
...
- For local role debugging, generate `requirements.yml` with an `APEXPLANE_ROLES_PATH` value that points at the local checkout:
  ...
  This installs the current local checkout, including uncommitted role changes, into the consumer repository collection path. Consumer wrappers should create local `requirements.yml` only when it does not already exist, so a selected source is preserved until the file is removed.
...
- Keep setup/update playbooks focused on desired state; run `bin/migrate apply` before `bin/run` when pending migrations should be previewed or applied.
