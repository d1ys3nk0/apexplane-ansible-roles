# ApexPlane Ansible Roles

Reusable, opinionated Ansible roles for Linux infrastructure.
...
For local role debugging, generate the consumer repository's local `requirements.yml` from a local checkout instead of Git:
...
- name: Configure hosts
  hosts: all
  roles:
    - role: apexplane.roles.base_bootstrap
    - role: apexplane.roles.base_packages
```
...
- `docs/`: role authoring, shared consumer docs, and contract documentation.
